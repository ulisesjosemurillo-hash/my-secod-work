...
object DataExtractor {
    ...
    fun extractPerjudicado(text: String): String {
        val cleanText = normalizeText(text)
        val regexes = listOf(
            Regex("""(?i)en perjuicio de(?:l| la)?\s+([A-ZÑÁÉÍÓÚ\s]+?)(?:;|\.|-|para el d[ií]a|a quien se le|orden[áa]ndosele|Art[íi]culo|$)"""),
        )
        for (regex in regexes) {
            val match = regex.find(cleanText)
            if (match != null) {
    ...
    fun extractJuez(text: String): String {
        val cleanText = normalizeText(text)
        val regexes = listOf(
            Regex("""(?i)JUEZ\s*:\s*([A-ZÑÁÉÍÓÚ ]{3,50})(?=\s|$)"""),
            Regex("""(?i)ABOG(?:ADA|ADO|\.)?\s+([A-ZÑÁÉÍÓÚ ]{3,50}?)\s*(?:-|–)?\s*JUEZ"""),
            Regex("""(?i)([A-ZÑÁÉÍÓÚ ]{3,50}?)\s*(?:-|–)?\s*JUEZ\s+JUZGADO""")
        )
        val lastPart = cleanText.takeLast(1000.coerceAtMost(cleanText.length))
        for (regex in regexes) {
    ...
    fun extractSecretario(text: String): String {
        val cleanText = normalizeText(text)
        val regexes = listOf(
            Regex("""(?i)SECRETARI[OA](?:\(A\))?\s*(?:ADJUNTO(?:\(A\))?)?\s*:\s*([A-ZÑÁÉÍÓÚ ]{3,50})(?=\s|$)"""),
            Regex("""(?i)ABOG(?:ADA|ADO|\.)?\s+([A-ZÑÁÉÍÓÚ ]{3,50}?)\s*(?:-|–)?\s*SECRETARI[OA]"""),
            Regex("""(?i)([A-ZÑÁÉÍÓÚ ]{3,50}?)\s*(?:-|–)?\s*SECRETARI[OA](?:\(A\))?\s*ADJUNTO""")
        )
        val lastPart = cleanText.takeLast(1000.coerceAtMost(cleanText.length))
        for (regex in regexes) {
            val match = regex.find(lastPart)
    ...
    fun parseOcrResult(text: String): ReprogramacionData {
        return ReprogramacionData(
            ...
            nuevaFecha = "",
            nuevaHora = "",
            nombreJuez = extractJuez(text),
            nombreSecretario = extractSecretario(text),
